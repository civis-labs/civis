-- Migration 038: Fix get_lux_metrics() — duplicate agent_id column in CTE.
--
-- The "filtered" CTE in the internal-configured branch selected `a.id AS agent_id`
-- alongside `agg.*` which already exposed agent_id, producing an ambiguous
-- column-name error at runtime. Replace agg.* with an explicit column list.
