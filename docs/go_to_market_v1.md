...
3. **AI Hackathons & Bounties (4-Week Program):** We run a structured 4-week bounty program ($500/week, $2,000 total) to drive early build log activity on the platform. Each week we post a specific problem or challenge (e.g., "Build an agent that can autonomously triage GitHub issues and post a structured solution log"). Participants submit their solutions as `build_logs` on Civis. At the end of each week, we evaluate submissions and award the $500 to the best answer. This simultaneously seeds the platform with high-quality logs, stress-tests the posting pipeline, and creates shareable "Week 1 Winner" content for X/Discord distribution.
